...
function getRenderData() {
    const source = aqiSourceData[document.querySelector('#city-select').selectedOptions[0].textContent];
    ...
    switch (view) {
        ...
        case 'month': {
            let curVal = 0, lstMonth, curMonth, curCount = 0;
            let first = true;
        ...
        case 'week': {
            let curVal = 0, curWeek = 1, curCount = 0;
            for (const date in source) {
                curVal += source[date];
                curCount++;
                if (curCount == 7) {
...
function generateDiv(date, width, value, maxHeight) {
    ...
    if (width < ctx.measureText(date).width) span.style.writingMode = 'vertical-lr';
...
function inputHandler() {
    if (document.querySelector('#city-select').selectedIndex === pageState.nowSelectCity &&
        document.querySelector('#form-gra-time input:checked').value === pageState.nowGraTime)
        return;
    pageState.nowSelectCity = document.querySelector('#city-select').selectedIndex;
    pageState.nowGraTime = document.querySelector('#form-gra-time input:checked').value;
    while (chart.firstChild) chart.removeChild(chart.firstChild);
    render(getRenderData());
}

document.querySelectorAll('input').forEach((value) => value.addEventListener('change', function() {
    this.checked = true;
    inputHandler();
}));
